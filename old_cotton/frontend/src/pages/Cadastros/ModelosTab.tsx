...
const modeloSchema = z.object({
  ...
  capacidadeMaximaFardos: z.coerce.number({ message: 'Deve ser um número' }).int().positive('Deve ser maior que 0'),
  pesoMaximoKg: z.coerce.number({ message: 'Deve ser um número' }).int().positive('Deve ser maior que 0'),
  metragem: z.coerce.number().optional().or(z.literal('')),
...
export default function ModelosPage() {
  ...
  function openModal(m?: any) {
    if (m) {
      ...
      setEditando(null);
      reset({ nomeDescricao: '', motoristaNome: '', motoristaTelefone: '', placaVeiculo: '', capacidadeMaximaFardos: undefined, pesoMaximoKg: undefined, metragem: undefined, observacoes: '' });
    }
    setModalOpen(true);
  }
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={() => openModal()}
          className="inline-flex items-center justify-center rounded bg-green-700 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-green-800">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-gray-100 px-6 py-4">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{editando ? 'Editar Veículo' : 'Cadastro de Veículos'}</h2>
                <p className="text-xs text-gray-500">Preencha os dados do motorista e do veículo.</p>
              </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" step="0.01" {...register('metragem')}
                    placeholder="Metros de carga"
                    className={`w-full border ${errors.metragem ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 text-sm focus:ring-1 focus:ring-blue-500 focus:outline-none`} />
                  {errors.metragem && <p className="text-[10px] text-red-500 mt-1">{errors.metragem.message}</p>}
                </div>
                <div className="md:col-span-2">
              ...
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button type="submit"
                  ...
                  {salvar.isPending ? 'Salvando...' : editando ? 'Salvar Alterações' : 'Cadastrar Veículo'}
                </button>
              </div>
            </form>
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {filtrarDados().map((m: any) => (
                  <tr key={m.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-right text-gray-700">{m.metragem ? `${m.metragem}m` : '—'}</td>
