...
export default function VeiculoModal({ liberacaoId, veiculoId, onClose, onSaved }: Props) {
  ...
  const [form, setForm] = useState<any>({
    liberacaoId, placa: '', modeloCarretaId: '',
  ...
  const modelos = Array.isArray(modelosData) ? modelosData : modelosData?.data || [];
  ...
  useEffect(() => {
    if (veiculoData) setForm({ ...veiculoData, modeloCarretaId: veiculoData.modeloCarretaId });
  }, [veiculoData]);
  ...
  function handleSave() {
    if (!form.modeloCarretaId) {
      toast.error('Selecione um Modelo de Carreta');
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl">
        ...
        <div className="p-6 grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-1">Modelo de Carreta*</label>
            <select value={form.modeloCarretaId || ''} onChange={(e) => set('modeloCarretaId', e.target.value ? parseInt(e.target.value) : '')}
              className="w-full border border-gray-200 rounded px-2 py-1.5 text-sm">
              <option value="">Selecionar...</option>
              {(modelos ?? []).map((m: any) => (
                <option key={m.id} value={m.id}>{m.nomeDescricao} (cap. {m.capacidadeMaximaFardos})</option>
              ))}
            </select>
          </div>
          ...
          <Field label="CPF Motorista" value={form.motoristaCpf ?? ''} onChange={(v: string) => set('motoristaCpf', v)} placeholder="Somente números" />
          <div>
