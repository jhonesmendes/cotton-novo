...
export default function OrigensTab() {
  const qc = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [confirmDeletar, setConfirmDeletar] = useState<any | null>(null);

  const { register, handleSubmit, reset, formState: { errors } } = useForm<OrigemFormData>({
    resolver: zodResolver(origemSchema),
  });

  const { data: response, isLoading } = useQuery({
    queryKey: ['origens'],
    queryFn: () => api.get('/origens').then((r) => r.data),
  });

  const origens = Array.isArray(response) ? response : response?.data || [];

  const salvar = useMutation({
    mutationFn: (data: OrigemFormData) => editingId ? api.put(`/origens/${editingId}`, data) : api.post('/origens', data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['origens'] });
      toast.success(editingId ? 'Filial/Origem atualizada!' : 'Filial/Origem criada!');
      handleCancel();
    },
  ...
  const deletar = useMutation({
    mutationFn: (id: number) => api.delete(`/origens/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['origens'] });
      toast.success('Filial/Origem removida!');
  ...
  function handleEdit(c: any) {
    setEditingId(c.id);
    reset({ nome: c.nome, localizacao: c.localizacao, estado: c.estado });
    setShowForm(true);
  }

  function handleCancel() {
    setEditingId(null);
    reset({ nome: '', localizacao: '', estado: '' });
    setShowForm(false);
  }

  const onSubmit = (data: OrigemFormData) => {
    salvar.mutate(data);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-bold text-gray-800">Lista de Filiais Embarcadoras</h2>
        <button
          onClick={() => { if (showForm) handleCancel(); else setShowForm(true); }}
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-medium"
        >
          {showForm ? 'Fechar Formulário' : <><Plus size={18} /> Nova Origem</>}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-xl border p-6 shadow-sm">
          <h3 className="text-md font-bold mb-4">{editingId ? 'Editar Origem' : 'Nova Origem'}</h3>
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input {...register('localizacao')} placeholder="Ex: PRIMAVERA DO LESTE"
                className={`w-full px-3 py-1.5 border ${errors.localizacao ? 'border-red-500' : 'border-gray-200'} rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500`} />
              {errors.localizacao && <p className="text-[10px] text-red-500 mt-1">{errors.localizacao.message}</p>}
            </div>
            <div>
      ...
      <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-gray-400">Nenhuma origem cadastrada.</div>
        ) : (
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="px-6 py-3 font-semibold text-gray-500">NOME</th>
                <th className="px-6 py-3 font-semibold text-gray-500">LOCALIZAÇÃO</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {origens.map((o: any) => (
                <tr key={o.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-gray-600">{o.localizacao}</td>
      ...
      <ConfirmModal
        ...
        title="Remover Filial/Origem"
        message={`Deseja remover a origem ${confirmDeletar?.nome}?`}
